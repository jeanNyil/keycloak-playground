  ...
  if (hasToken) {
    try {
      ...
      if (tokenParts.length !== 3) {
        ...
        const hasCorrectAudience = audiences.includes('oauth-backend');
        if (!hasCorrectAudience) {
          console.log(`  └─ ⚠️  Token audience mismatch (expected: oauth-backend, got: ${JSON.stringify(audiences)})`);
        }
        
        // Check if user has required role
        const clientRoles = payload.resource_access?.['oauth-backend']?.roles || [];
...
app.get('/secured', keycloak.protect('oauth-backend:user'), function (req, res) {
  ...
  if (grant) {
    ...
    const roles = token.content.resource_access?.['oauth-backend']?.roles || [];
...
app.listen(3000, function () {
  ...
  console.log('  - GET /secured (requires oauth-backend:user role)');

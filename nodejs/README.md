...
- **OIDC Client**: For the OIDC Playground
  - Client ID: `nodejs-oidc-playground`
  - Access Type: `public`
...
>
> **Important**: After importing the `nodejs-oauth-backend` client configuration, you must manually create the `user` client role in the Keycloak Admin UI, as client roles cannot be imported via the UI:
> 1. Navigate to **Clients** → **nodejs-oauth-backend** → **Roles** tab
> 2. Click **Create Role**
> 3. Set **Role Name** to `user`
> 4. Save the role
> 5. Assign this role to your test user via **Users** → *[select user]* → **Role Mapping** → **Client Roles** → **nodejs-oauth-backend**
...
service.name = "nodejs-oidc-playground"
service.name = "nodejs-oauth-playground-frontend"
service.name = "nodejs-oauth-playground-backend"
```
